...
const choose = (n: number, d: number) => fac(n) / (fac(d) * fac(n - d))

export const rollProp = (n: number, d: number) => {
  console.log('n:', n, ' d:', d)
  return (p: number) => {
    const c = 1 / Math.pow(d, n)
    const kmax = Math.floor((p - n) / d)
    let sum = 1
    for (let k = 1; k <= kmax; k++) {
      sum =
        ...
        Math.pow(-1, k) *
          choose(n, k) *
          choose((p - d) * (k - 1), (p - d) * (k - n))
    }
    console.log(kmax, sum)
    return c * sum
  }
}
